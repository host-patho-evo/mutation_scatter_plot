...
class TestMutationScatterPlot(unittest.TestCase):
    ...
    def test6_disable_padded_x_axis(self):
        """Verify that --disable-padded-x-axis properly shrinks axis and suppresses padded position in Bokeh JS."""
        with tempfile.TemporaryDirectory() as tmpdir:
            full_tsv = os.path.join(self.outputs_dir, "test2_full.x_after_count.frequencies.tsv")
            target_prefix = "test_run_test6"
            outfile_prefix = os.path.join(tmpdir, target_prefix)
            cmd_args = [
                "--tsv", full_tsv,
                "--outfile-prefix", outfile_prefix,
                "--show-STOP", "--show-X", "--show-DEL", "--show-INS",
                "--disable-padded-x-axis",
                "--disable-showing-bokeh", "--disable-showing-mplcursors"
            ]
            returncode, output = self._invoke_cli(cmd_args)
            self.assertEqual(returncode, 0, f"Command with --disable-padded-x-axis failed:\n{output}")

            # Check the HTML file ensuring it injected the mapping override JS
            html_files = [f for f in os.listdir(tmpdir) if f.endswith(".html")]
            self.assertGreater(len(html_files), 0, "No HTML output generated")
            html_path = os.path.join(tmpdir, html_files[0])
            with open(html_path, encoding="utf-8") as fh:
            ...
            self.assertIn(
                "return String(mapping[padded]);", html_text,
                "Bokeh HTML did not contain the override for natural X-axis labeling"
            )
            # The JS code snippet without disable-padded-x-axis contains:
            # return String(padded) + " (" + mapping[padded] + ")";
            self.assertNotIn(
                'return String(padded) + " (" + mapping[padded] + ")";', html_text,
                "Bokeh HTML illegally embedded the padded axis formatter despite toggle"
            )


    def test7_bokeh_strict_axis_bounds(self):
        """Verify that Bokeh HTML payload purely isolates data inside [xmin, xmax], restricting
        elements accurately to spatial boundaries."""
        with tempfile.TemporaryDirectory() as tmpdir:
            full_tsv = os.path.join(self.outputs_dir, "test2_full.x_after_count.frequencies.tsv")
            target_prefix = "test_run_test7"
            outfile_prefix = os.path.join(tmpdir, target_prefix)
            ...
            returncode, output = self._invoke_cli(cmd_args)
            self.assertEqual(returncode, 0, f"Command with xmin/xmax failed:\n{output}")

            html_files = [f for f in os.listdir(tmpdir) if f.endswith(".html")]
            self.assertGreater(len(html_files), 0, "No HTML output generated")
            html_path = os.path.join(tmpdir, html_files[0])

            # Extract JSON payload arrays from Bokeh
            extracted_bokeh_data = self._extract_bokeh_data(html_path)
            self.assertGreater(len(extracted_bokeh_data), 0, "No ColumnDataSource found in Bokeh output")

            for keys, rows in extracted_bokeh_data:
                try:
                    padded_idx = keys.index("padded")
                except ValueError:
                    continue  # Skip sources that lack positional data

                for r in rows:
                    _val = int(float(r[padded_idx]))
                    self.assertTrue(
                        _xmin <= _val <= _xmax,
                        f"Axis bounding violated! Embedded coordinate {_val} falls outside strict requested bounds [{_xmin}, {_xmax}]."
                    )
