...
myparser.add_option("--colormap", action="store", type="string", dest="colormap", default='amino_acid_changes',
...
def alt_translate(seq):
    ...
    _codons = (seq[i:i+3] for i in range(0, len(seq), 3))
    _codons = ("NNN" if "-" in _codon and _codon != "---" else _codon for _codon in _codons)
    return "".join(translate(_codon, gap='-') for _codon in _codons)
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if myoptions.colormap == 'microshades_cvd_palettes':
                                        ...
                                        # colors in total
                                        _colors1 = plt.cm.gist_heat(np.linspace(0.2, 0.8, 9)) # standard matplotlib.cm colormap
                                        _colors2 = plt.cm.cividis_r(np.linspace(0.2, 0.8, 9)) # standard matplotlib.cm colormap
                                        _colors3 = plt.cm.GnBu_r(np.linspace(0.2, 0.8, 9)) # standard matplotlib.cm colormap
                                        ...
                                        # sample the colormaps that you want to use. Use 128 from each so we get 256
                                        # colors in total
                                        _colors1 = plt.cm.viridis(np.linspace(0.2, 0.8, 9)) # standard matplotlib.cm colormap
                                        _colors2 = plt.cm.coolwarm_r(np.linspace(0., 1, 9)) # standard matplotlib.cm colormap
                                        _colors3 = plt.cm.GnBu_r(np.linspace(0.2, 0.8, 9)) # standard matplotlib.cm colormap
                                        ...
                                        # _amino_acid_changes_cmap = ["#F5F5F5", "#D6D6D6", "#B7B7B7", "#8B8B8B", "#ff0000", "#f16264", "#ffc8ff", "#EFB6D6", "#CC79A7", "#ff9900", "#F09163", "#9c644b", "#ffcc00", "#cccc00", "#A3E4D7", "#7DCCFF", "#0042ff", "#DDFFA0", "#97CE2F", "#219f11", "#00ff04", "#c20078", "#ff00fd", "#c500ff", "#8200ff", "#960000", "#580041"]
                                        # with yellow "#ffff00" in the middle _colors[20]
                                        _colors = ["#930000", "#930000", "#930000", "#930000", "#930000", "#930000", "#960000", "#580041", "#8200ff", "#c500ff", "#ff00fd", "#CC79A7", "#eea1d0", "#cc0000", "#ff0000", "#ff4f00", "#ff7c7c", "#ff9999", "#c58a24", "#9c644b", "#ffff00", "#ffcc00", "#ffa200", "#7DCCFF", "#0042ff", "#0000ff", "#D6D6D6", "#B7B7B7", "#8B8B8B", "#bbff00", "#00ff04", "#97CE2F", "#219f11", "#930000", "#930000", "#930000", "#930000", "#930000", "#930000"]
                                        _cmap = matplotlib.colors.ListedColormap(_colors, "amino_acid_changes", len(_colors)) # 39 colors
                                        #plt.cm.register_cmap(name='amino_acid_changes', cmap=_cmap)
                                        myoptions.colormap = 'amino_acid_changes'
                                        _norm = matplotlib.colors.BoundaryNorm(np.arange(-19, 19, 1), _cmap.N) # this will create an asymetric range -19 to 18
                                        return _norm, _cmap, _colors

    return _norm, _cmap, _colors


def resolve_codon_or_aa(old_codon_or_aa, new_codon_or_aa):
...
def get_score(matrix, codon_on_input, old_codon_or_aa, new_codon_or_aa):
    try:
        if codon_on_input:
        ...
        raise ValueError("Cannot get a score for myoptions.matrix='%s', old_codon_or_aa='%s', new_codon_or_aa='%s'" % (myoptions.matrix, old_codon_or_aa, new_codon_or_aa))
    return _score


def adjust_size_and_color(frequency, codon_on_input, old_codon_or_aa, new_codon_or_aa, _old_codon_or_aa, _new_codon_or_aa, matrix, norm, colors):
    ...
    if _new_codon_or_aa in ('---', 'DEL', 'INS'):
        ...
        _score = get_score(matrix, codon_on_input, _old_codon_or_aa, _new_codon_or_aa)

    _colorindex = norm(_score) # this only affects figures rendered by Bokeh, not by Matplotlib

    # if one rewrites _score with a new value it:
    #   1. makes the data point in green in Matplotlib images
    #   2. the fake score 12 is shown in Bokeh images during hover()
    #   3. the original score is shown in Matplotlib images during hover() and mplcursors
    # In summary, we need this enabled if we want to emphasize synonymous mutations to make image more readable.
    if old_codon_or_aa.upper() == new_codon_or_aa.upper():
        _color = '#00ff04' # use palegreen for synonymous changes
        _score = 12 # enter fake score because matplotlib.scatter ignores the colors passed to it when cmap and norm are passed to it as well
        ...
        _color = 'red'
        _score = -6 # enter fake score because matplotlib.scatter ignores the colors passed to it when cmap and norm are passed to it as well
        ...
        if alt_translate(_old_codon_or_aa) == alt_translate(_new_codon_or_aa):
            ...
            _color = colors[_colorindex] # get the color by its index position in the ListedColormap
            if myoptions.debug:
...
def adjust_size_and_color_neutralized_escape(neutralized_parent_difference, codon_on_input):
    ...
    if neutralized_parent_difference < -0.001:
        ...
        if not codon_on_input:
            _size, _color = 0, '#00ff04' # do not draw circles for unchanged amino acids
        else:
...
def adjust_size_and_color_weighted(weighted_diff_escape_neutralized, generic_circle_size=5000, weighted_diff_escape_neutralized_size1=3000, weighted_diff_escape_neutralized_size2=2000): # use for 'weighted_diff_escape_neutralized'
...
def load_and_clean_dataframe(outfile_prefix):
    ...
    if myoptions.showstop:
        ...
        print("Info: Originally there were %d rows but after discarding codons with [N n DEL] there are only %d left" % (_before, _after))
    # print(df)
    print(f"Info: Writing into {outfile_prefix}.actually_rendered.tsv")
    df.to_csv(f"{outfile_prefix}.actually_rendered.tsv", sep='\t', header=None, index=False, float_format='{:7.6f}'.format) # dump back the edited dataframe for eventual checks and disable scientific notation of float numbers
    if '.frequencies.tsv' in myoptions.tsv_file_path:
        _count_filename = outfile_prefix + '.count'
    ...
    return df, title_data, _aln_rows
...
def build_frequency_tables(df):
    ...
    #_amino_acids = ['A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H', 'I', 'L', 'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V', '*']
    _amino_acids = ['C', 'R', 'K', 'E', 'Q', 'D', 'N', 'T', 'S', 'H', 'M', 'P', 'W', 'Y', 'F', 'V', 'L', 'I', 'A', 'G'] # 'INS', 'X', 'DEL'] #, None] # sorted by physicochemical properties
    ...
    _unique_aa_positions = [x for x in df['position'].unique()]
    _min_aa_pos = min(_unique_aa_positions)
    _max_aa_pos = max(_unique_aa_positions)
    ...
    _unique_aa_positions = [x for x in range(_min_aa_pos, _max_aa_pos + 1 + _number_of_insertions)] # ensure we have a continuous range, eventually inject INS positions, add +1 to accommodate for the pandas dataframe index column
    if myoptions.debug:
        print(f"Debug: len(_unique_aa_positions)={len(_unique_aa_positions)}, _unique_aa_positions: {str(_unique_aa_positions)}")
    _unique_codon_positions = list(_unique_aa_positions)
    if myoptions.debug:
        print(f"Debug: len(_unique_codon_positions)={len(_unique_codon_positions)}, _unique_codon_positions: {str(_unique_codon_positions)}")
    ...
    # ensure the resulting _codons_whitelist2 will have same length as the codons later parsed from TSV file into _table.index
    ...
    # >>> [_tuple for x in _amino_acids for _tuple in _sorted_whitelist if _tuple[1] == x]
    ...
    _codons_whitelist2 = [x[0] for x in _final_sorted_whitelist] # get back the list of codons in the order used in _final_sorted_whitelist but also in sync with the order on Y-axis further down
    ...
    #}
    _old_aa_table = pd.DataFrame(Decimal(0), index=_amino_acids, columns=_unique_aa_positions)
    _new_aa_table = pd.DataFrame(Decimal(0), index=_amino_acids, columns=_unique_aa_positions)
    _old_codon_table = pd.DataFrame(Decimal(0), index=_codons_whitelist2, columns=_unique_codon_positions)
    _new_codon_table = pd.DataFrame(Decimal(0), index=_codons_whitelist2, columns=_unique_codon_positions)
    ...
    for df_index, row in df.iterrows():
        ...
        # in codons list. Skipping some rows would make _new_aa_table and _new_codon_table
        # have a different amount of rows, breaking slicing
        _position = row['position'] # real position_in_protein
        if _very_leftmost_aa_pos is None:
            _very_leftmost_aa_pos = int(_position)
        ...
        _old_amino_acid = row['original_aa']
        _new_amino_acid = row['mutant_aa']
        frequency = Decimal(row[myoptions.column_with_frequencies])
        _old_codon = row['original_codon'].upper()
        _new_codon = row['mutant_codon'].upper()

        if (not myoptions.aminoacids or _old_amino_acid != _new_amino_acid or myoptions.include_synonymous) and not np.abs(frequency) < myoptions.threshold: # discard sequencing noise by zapping the values instead of deleting altogether
            _old_value = Decimal(0)
            try:
                _old_value = _new_aa_table.at[_new_amino_acid, _position]
            except KeyError:
                _new_aa_table.at[_new_amino_acid, _position] = Decimal(frequency)
            except TypeError:
                raise TypeError("Weird value %s" % _new_aa_table.at[_new_amino_acid, _position])
            else:
                _new_aa_table.at[_new_amino_acid, _position] = Decimal(_old_value) + Decimal(frequency) # the _position with the new amino acid may appear multiple times so sum up the values

            _old_value = Decimal( 0)
            try:
                _old_value = _old_aa_table.at[_old_amino_acid, _position]
            except KeyError:
                _old_aa_table.at[_old_amino_acid, _position] = Decimal(frequency)
            except TypeError:
                raise TypeError("Weird value %s" % _old_aa_table.at[_old_amino_acid, _position])
            else:
                _old_aa_table.at[_old_amino_acid, _position] = Decimal(_old_value) + Decimal(frequency) # the _position with the original amino acid will appear multiple times so sum up the values

            _old_value = Decimal(0)
            try:
                _old_value = Decimal(_old_codon_table.at[_old_codon, _position])
            except KeyError:
                _old_codon_table.at[_old_codon, _position] = Decimal(frequency) # the _position with the original codon will appear multiple times so sum up the values
            except TypeError:
                raise TypeError("Weird value %s" % _old_codon_table.at[_old_codon, _position])
            else:
                _old_codon_table.at[_old_codon, _position] = Decimal(_old_codon_table.at[_old_codon, _position]) + Decimal(frequency) # rewrite np.float64 into Decimal

            _old_value = Decimal(0)
    #        try:
    #            _new_codon_table.at[_new_codon, _position] = Decimal(_new_codon_table.at[_new_codon, _position]) + Decimal(frequency)
    #        except KeyError:
            _new_codon_table.at[_new_codon, _position] = Decimal(frequency) # the _position and a codon should appear just once so no reason to sum up new value with previous
    #        except TypeError:
    #            # sanitize nan values in the table
    #            raise TypeError("Weird value %s" % _new_codon_table.at[_new_codon, _position])
            if myoptions.debug:
                print(f"Debug: OriginalDataFrameRowNumber: {df_index}, Old: {_old_amino_acid}, New: {_new_amino_acid}, Frequency: {frequency}")
...
def setup_matplotlib_figure(
    ...
    final_sorted_whitelist, unique_aa_positions, unique_codon_positions,
    ...
    # aln_rows contain number of rows in the ALN file typically each read is counted so roughly divide it by two to get at about number of amplicons sequenced
    if myoptions.aminoacids:
        if myoptions.shortlegend:
            _xlabel = 'Amino acid position'
        else:
            _xlabel = 'Amino acid position%sbased on %s ALN rows, matrix %s, colormap %s, mutation_scatter_plot.py %s' % (os.linesep, aln_rows.strip(os.linesep), matrix_name, myoptions.colormap, VERSION)
    else:
        if myoptions.shortlegend:
            _xlabel = 'Codon position'
        else:
            _xlabel = 'Codon position%sbased on %s ALN rows, matrix %s, colormap %s, mutation_scatter_plot.py %s' % (os.linesep, aln_rows.strip(os.linesep), matrix_name, myoptions.colormap, VERSION)
    ...
    #    ax.set_xticks(np.arange(0, max(unique_codon_positions), round(round(max(unique_aa_positions)/20)/10.0)*10)) # this breaks syncing of ax and ax2 axes to be out-of-sync, worked around via plt.locator_params()
        ...
        _ax1.set_yticks(_y_ticks)
        _ax1.set_yticklabels([_pairs[0] + ' (' + _pairs[1] + ')  ' if _pairs[1] not in ('INS', 'DEL') else _pairs[0] + ' (' + _pairs[1] + ')' for _pairs in final_sorted_whitelist], fontsize=8) # sorted by amino acids
    ...
    if not myoptions.disable_2nd_Y_axis:
        ...
        if myoptions.aminoacids:
            _ax2.bar(unique_aa_positions, _total_frequencies, color='black', alpha=0.5, width=0.8, align='center') # , label='AA Frequencies')
        else:
            _ax2.bar(unique_codon_positions, _total_frequencies, color='black', alpha=0.5, width=0.8, align='center') # , label='Codon Frequencies')
...
def collect_scatter_data(
    ...
    amino_acids, codons_whitelist2,
):
    "Iterate over frequency tables and collect scatter plot data, labels, and colors."

    #for i, _some_codon_or_aa in enumerate(table.index): # causes off-by-one error
    #    for j, _aa_position in enumerate(table.columns): # causes off-by-one error

    _real_aa_positions = list(table.columns)
    if myoptions.debug:
        print(f"Debug: _real_aa_positions={str(_real_aa_positions)}")

    if not myoptions.aminoacids:
        if list(table.index) != codons_whitelist2: # ignore codons with 'N'
            raise ValueError("Both lists should be equal: %s=%s, %s=%s" % ('table.index', str(table.index), 'codons_whitelist2', codons_whitelist2))
    else:
        if list(table.index) != amino_acids: # check if the input were parsed correctly and codons with Ns dropped resulting in X amino acids
            raise ValueError("Both lists should be equal: %s=%s, %s=%s" % ('table.index', str(table.index), 'amino_acids', amino_acids))

    _used_colors = set()
    _norm, _cmap, _colors = get_colormap(myoptions.colormap)
    ...
    _mutations = []
    _circles_bokeh = [] # scaled later by 100 for bokeh figures
    _circles_matplotlib = [] # scaled later by 5000 for matplotlib figures
    _markers = []
    _dots = []
    _warn_once = []
    ...
    for i, _some_codon_or_aa in enumerate(table.index): # introduces off-by-one error, iterate over amino_acids which were used as the index using 'pd.DataFrame(Decimal(0), index=amino_acids, columns=unique_aa_positions)'
        for j, _aa_position in enumerate(table.columns): # introduces off-by-one error
            if myoptions.debug:
                print(f"Debug: i: {str(i)}, j: {str(j)}, _aa_position column: {str(_aa_position)}")
            ...
            _frequency = table.loc[_some_codon_or_aa, _aa_position]
            if myoptions.debug and _frequency:
                # Debug0: _aa_position=351, _some_codon_or_aa=C, _frequency=0.00109599999999999996903865540076594697893597185611724853515625
                # 7-SA-AA.SAPA@SA-Lib_F+SA-Lib_R.None+None.Wuhan.gofasta.frequencies.tsv
                # 351	Y	C	0.001096	TAT	TGT
                print(f"Debug0: _aa_position={_aa_position}, _some_codon_or_aa={_some_codon_or_aa}, _frequency={str(_frequency)}")
            try:
                ...
                if _frequency and myoptions.debug:
                    print(f"Debug0b: _aa_position={_aa_position}, _some_codon_or_aa={_some_codon_or_aa}, _frequency={str(_frequency)}")
                ...
                next
            _codon_on_input, _old_codon_or_aa, _new_codon_or_aa = resolve_codon_or_aa(_old_codon, _some_codon_or_aa)
            _score = None
            if not np.abs(Decimal(_frequency)) < myoptions.threshold:
                if myoptions.column_with_frequencies in ['neutralized_parent_difference', 'escape_parent_difference']:
                    _size, _color = adjust_size_and_color_neutralized_escape(Decimal(_frequency))
                    _score = -12 # TODO: junk value
                    ...
                    _score, _size, _color = adjust_size_and_color(Decimal(_frequency), _codon_on_input, _old_codon, _some_codon_or_aa, _old_codon_or_aa, _new_codon_or_aa, matrix, _norm, _colors) # the _color was derived via np.linspace mapping to range(0,1)
                    _matrix_values.add(_score) # update the set() with unique score values actually present in the dataset
                if myoptions.debug:
                    print(f"Debug: Real AA position: {_aa_position}, observed codon: {_some_codon_or_aa}, _frequency: {_frequency}, _size: {_size}, color: {_color}")
                ...
                if myoptions.aminoacids:
                    if _score < 0:
                        _circles_bokeh.append((_aa_position, _some_codon_or_aa, float(np.abs(_size) * 100), 'circle', _color, 0.5, _score))
                    else:
                        _circles_bokeh.append((_aa_position, _some_codon_or_aa, float(np.abs(_size) * 100), 'hex', _color, 0.5, _score))
                else:
                    # print later codons followed by the amino acid they encode on Y-axis
                    # matplotlib cannot render in one go using multiple markers
                    # https://stackoverflow.com/questions/75676193/using-different-markers-in-a-scatter-plot#75677203
                    if _score < 0:
                        _circles_bokeh.append((_aa_position, _some_codon_or_aa + ' (' + alt_translate(_some_codon_or_aa) + ')', float(np.abs(_size) * 100), 'circle_x', _color, 0.5))
                    else:
                        _circles_bokeh.append((_aa_position, _some_codon_or_aa + ' (' + alt_translate(_some_codon_or_aa) + ')', float(np.abs(_size) * 100), 'hex', _color, 0.5))
                ...
                if _score < 0:
                    ...
                    _circles_matplotlib.append((_aa_position, i, float(np.abs(_size) * 5000), 'circle_x', _color, 0.5, _score, 'o'))
                    ...
                    # marker='h' or 'H' will use hexagon or hexagon2
                    _circles_matplotlib.append((_aa_position, i, float(np.abs(_size) * 5000), 'circle', _color, 0.5, _score, 'h'))
                ...
                _size, _color = 0, 'black' # if we force the _size to 0.00000000009 the invisible dots are drawn in matplotlib figs
                _score = get_score(matrix, _codon_on_input, _old_codon_or_aa, _new_codon_or_aa)
                # for matplotlib figures use i index
                _dots.append((_aa_position, i, _size, 'dot', _color, 0.5, _score))
                if myoptions.debug:
                    print(f"Debug: Invisible dot. Real AA position: {_aa_position}, observed codon: {_some_codon_or_aa}, _frequency: {_frequency}, _size: {_size}, color: {_color}")
            # df.columns = ['position', 'original_aa', 'mutant_aa', '_frequency', 'original_codon', 'mutant_codon']


            # prepare labels for all datapoints
            # discard sequencing noise
            if _aa_position not in _warn_once:
                _frequencies = [Decimal(x) for x in df.loc[(df['position'] == _aa_position) & (df['mutant_aa'] == _some_codon_or_aa) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)][myoptions.column_with_frequencies].to_list()] # convert strings to Decimal values
                try:
                    _old_amino_acid = df.loc[df['position'] == _aa_position]['original_aa'].to_list()[0] # this sometimes fails to slice 0 element
                except IndexError:
                    print(f"Error: Cannot slice {str(df.loc[df['position'] == _aa_position]['original_aa'].to_list())}")
                    _old_amino_acid = df.loc[df['position'] == _aa_position]['original_aa'].to_list()[0]

                _new_codons = df.loc[(df['position'] == _aa_position) & (df['mutant_aa'] == _some_codon_or_aa) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)]['mutant_codon'].to_list()
                if myoptions.aminoacids:
                    if _frequency != table.at[_some_codon_or_aa, _aa_position]:
                        raise ValueError("Values _frequency=%s and table.at[_some_codon_or_aa, _aa_position]=%s should be equal" % (_frequency, table.at[_some_codon_or_aa, _aa_position])) # both approaches should work
                    ...
                    # filtered_codons, filtered_frequencies, _observed_codon_count_sum = filter_codons(_some_codon_or_aa, _new_codons, _frequencies, _observed_codon_counts, myoptions.threshold)
                    if not _frequency < myoptions.threshold: # append to the lists only if the _frequency is above threshold
                        ...
                        _html_labels.append(f"Position: {_aa_position}<br>Original Amino Acid: {_old_amino_acid} ({_old_codon})<br>New Amino Acid: {_some_codon_or_aa} {_new_codons}<br>{myoptions.matrix} score: {_score}<br>Cumulative Frequency: {sum(_frequencies):.6f}<br>Codon Frequencies: {['{:.6f}'.format(x) for x in _frequencies]}<br>Observed codon counts: {_observed_codon_counts}<br>Observed codon count sum: {_observed_codon_count_sum}<br>Total codons per site: {_total_codons_per_site}")
                    ...
                    if myoptions.debug:
                        ...
                        print(f"Info: {len(df.loc[df['position'] == _aa_position]['position'])} aa residues observed in position {_aa_position}:{os.linesep} {str(df.loc[df['position'] == _aa_position][0:])}{os.linesep}")
                    #print(f"Info: Looking for new_codon={_some_codon_or_aa}")
                    try:
                        _new_amino_acid = df.loc[(df['position'] == _aa_position) & (df['mutant_codon'] == _some_codon_or_aa)]['mutant_aa'].to_list()[0]
                    except:
                        # _new_amino_acid = "Failed to find, the %s is wrong and too far from original codon" % _some_codon_or_aa
                        _new_amino_acid = None

                    if _new_amino_acid:
                        try:
                            _some_frequency = Decimal(df.loc[(df['position'] == _aa_position) & (df['mutant_codon'] == _some_codon_or_aa)][myoptions.column_with_frequencies].to_list()[0]) # convert strings to Decimal values
                        except:
                            _some_frequency = 0.00000000009 # enter junk value but prevent matplotlib from crashing
                        ...
                        if _old_amino_acid and not _frequency < myoptions.threshold and _size:
                            ...
                            # also do not draw data points if the limits were applied manually with _size=0 and #00ff04 color enforced
                            ...
                            if myoptions.column_with_frequencies == 'neutralized_parent_difference':
                                ...
                                _html_labels.append(f"Position: {_aa_position}<br>Original Codon: {_old_codon} ({_old_amino_acid})<br>New Codon: {_some_codon_or_aa} ({_new_amino_acid})<br>{myoptions.matrix} score: {_score}<br>Difference neutralized2parent: {_frequency:.6f}")
                            elif myoptions.column_with_frequencies == 'escape_parent_difference':
                                _labels.append(f"Position: {_aa_position}\nOriginal Codon: {_old_codon} ({_old_amino_acid})\nNew Codon: {_some_codon_or_aa} ({_new_amino_acid})\n{myoptions.matrix} score: {_score}\nDifference escape2parent: {_frequency:.6f}")
                                _html_labels.append(f"Position: {_aa_position}<br>Original Codon: {_old_codon} ({_old_amino_acid})<br>New Codon: {_some_codon_or_aa} ({_new_amino_acid})<br>{myoptions.matrix} score: {_score}<br>Difference escape2parent: {_frequency:.6f}")
                                ...
                                _html_labels.append(f"Position: {_aa_position}<br>Original Codon: {_old_codon} ({_old_amino_acid})<br>New Codon: {_some_codon_or_aa} ({_new_amino_acid})<br>{myoptions.matrix} score: {_score}<br>Weighted difference escape2neutralized: {_frequency:.6f}")
                            elif myoptions.column_with_frequencies == 'frequency':
                                _labels.append(f"Position: {_aa_position}\nOriginal Codon: {_old_codon} ({_old_amino_acid})\nNew Codon: {_some_codon_or_aa} ({_new_amino_acid})\n{myoptions.matrix} score: {_score}\nFrequency: {_frequency:.6f}\nObserved codon count: {_observed_codon_counts}\nTotal codons per site: {_total_codons_per_site}")
                                _html_labels.append(f"Position: {_aa_position}<br>Original Codon: {_old_codon} ({_old_amino_acid})<br>New Codon: {_some_codon_or_aa} ({_new_amino_acid})<br>{myoptions.matrix} score: {_score}<br>Frequency: {_frequency:.6f}<br>Observed codon counts: {_observed_codon_counts}<br>Total codons per site: {_total_codons_per_site}")
                                _label_codon_positions.append(f"{_aa_position}")
                                _label_original_amino_acids.append(f"{_old_amino_acid} ({_old_codon})")
...
#                            _labels.append(f"Position: {_aa_position}\nOriginal Codon: {_old_codon} ({_old_amino_acid})\nNew Codon: {_some_codon_or_aa} ({_new_amino_acid})\nFrequency: {_frequency:.6f}")
#                            _html_labels.append(f"Position: {_aa_position}<br>Original Codon: {_old_codon} ({_old_amino_acid})<br>New Codon: {_some_codon_or_aa} ({_new_amino_acid})<br>Frequency: {_frequency:.6f}")
#                    else:
#                        # fill in the labels to get same amount of items in the list
#                        try:
#                            _frequency = Decimal(df.loc[(df['position'] == _aa_position) & (df['mutant_codon'] == _some_codon_or_aa)][myoptions.column_with_frequencies].to_list()[0])
...
#                        _html_labels.append(f"Position: {_aa_position}<br>Original Codon: {_old_codon} ({_old_amino_acid})<br>New Codon: {_some_codon_or_aa} ({_new_amino_acid})<br>Frequency: {_frequency}")


                # write colors used while in amino acid mode
                _position_in_protein = _aa_position
                _original_aas = df.loc[(df['position'] == _position_in_protein)]['original_aa'].to_list()
                if _original_aas:
                ...
                if myoptions.aminoacids:
                    _mutant_codons = df.loc[(df['position'] == _position_in_protein) & (df['mutant_aa'] == _new_amino_acid)]['mutant_codon'].to_list() # this returns empty list if not running in amino acid mode
                else:
                    ...
                    _mutant_codons = df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _some_codon_or_aa)]['mutant_codon'].to_list()
                if _original_aa and not _frequency < myoptions.threshold and _size:
                    ...
                    if len(_mutant_codons) > 1:
                        ...
                        # sort -t$'\t' -k 1,1n -k 8,8n -k 6,6n prefix.gofasta.aa._frequencies.colors.tsv
                    elif _mutant_codons:
    ...
    # _merged_lists = _circles_bokeh # _markers + _dots # do not add _markers as they are just the central dots inside circles_bokeh
    if myoptions.debug:
...
def render_bokeh(
    ...
    circles_bokeh, labels, html_labels, mutations,
    ...
    _mysource = bokeh.models.ColumnDataSource(data=dict(
        ...
        s=[s[2] for s in circles_bokeh], # _size
        m=[m[3] for m in circles_bokeh], # marker
        c=[c[4] for c in circles_bokeh], # _color
        a=[a[5] for a in circles_bokeh], # alpha
        label=labels,
        label1=label_codon_positions,
    ...
    if myoptions.aminoacids:
        _p = bokeh.plotting.figure(x_range=(xmin, xmax), y_range=amino_acids, tooltips=TOOLTIPS, title=title_data, x_axis_label=xlabel, y_axis_label='Introduced amino acid changes', x_minor_ticks=10, width=2000, height=1200, sizing_mode='stretch_width') # width=5000, height=5000)
    else:
        # codons sorted by amino acids
        _p = bokeh.plotting.figure(x_range=(xmin, xmax), y_range=[_pairs[0] + ' (' + _pairs[1] + ')' for _pairs in final_sorted_whitelist], tooltips=TOOLTIPS, title=title_data, x_axis_label=xlabel, y_axis_label='Introduced codon changes', x_minor_ticks=10, height=1200, width=2000, sizing_mode='stretch_width') # width=5000, height=5000)

    # print(f"ticks={str(_ticks)}")
    # _p.xaxis.ticker = _x_ticks
    #
    ...
    _p.axis.major_label_text_font_size = "12px" # major_label_text_font_size, axis_label_text_font_size or major_label_text_font_style
    _bokeh_scatter = _p.scatter(x='x', y='y', size='s', marker='m', color='c', alpha='a', source=_mysource)
...
def render_matplotlib(
    ...
    #_old_amino_acid =  df.loc[df['position'] == 145][1].to_list()[0]
    #print(f"Original AA at position {index_position_in_TSV} was {_old_amino_acid}")

    # https://stackoverflow.com/questions/75676193/using-different-markers-in-a-scatter-plot
    # https://github.com/matplotlib/matplotlib/issues/11155
    # _mpl_scatterplot = ax1.scatter([x[0] for x in circles_matplotlib], [x[1] for x in circles_matplotlib], marker=[x[7] for x in circles_matplotlib], s=[x[2] for x in circles_matplotlib], alpha=0.5, c=[x[6] for x in circles_matplotlib], cmap=cmap, norm=norm)
    ...
    _colorbar.ax.set_yticks(np.arange(-18.5, 18.5, 1), np.arange(-19, 18, 1)) # although the scalebar has labels from -19 to 17, the colors got shifted and match the input score values
    # _colorbar.ax.set_yticks(np.arange(-18.5, 19.5, 1), np.arange(-19, 20, 1))
    # ValueError: The number of FixedLocator locations (38), usually from a call to set_ticks, does not match the number of labels (39).
    # _colorbar.ax.set_yticks(np.arange(-18.5, 18.5, 1), np.arange(-19, 20, 1))
    # ValueError: The number of FixedLocator locations (37), usually from a call to set_ticks, does not match the number of labels (39).
    # _colorbar.ax.set_yticks(np.arange(-18.5, 18.5, 1), np.arange(-18, 18, 1))
    ...
    ax1.scatter([x[0] for x in markers], [x[1] for x in markers], s=[x[2] for x in markers], marker='x', color='black', alpha=0.5) # TODO: this applies same marker='x' to negative and positive values, somehow cannot find a way to utilize [x[3] for x in markers] containing a mix of prepared dots and circles
    ...
    _cursor = mplcursors.cursor(ax1, hover=True)
    if myoptions.aminoacids:
        @_cursor.connect("add")
        def on_add(sel):
            ypos, xpos = int(sel.target[1]), int(sel.target[0])
            #_old_amino_acid = old_aa_table.index[ypos]
            _new_amino_acid = new_aa_table.index[ypos]
            _position_in_protein = new_aa_table.columns[xpos+1-myoptions.offset-calculated_aa_offset] # undo the off-by-one error removal attempt on the ax.scatter line
            #print(f"Position: {_position_in_protein}")
            _frequency = new_aa_table.at[_new_amino_acid, _position_in_protein]
            _frequencies = [Decimal(x) for x in df.loc[(df['position'] == _position_in_protein) & (df['mutant_aa'] == _new_amino_acid) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)][myoptions.column_with_frequencies].to_list()] # convert strings to Decimal values
            _old_amino_acid = df.loc[df['position'] == _position_in_protein]['original_aa'].to_list()[0]
            _old_codon = df.loc[df['position'] == _position_in_protein]['original_codon'].to_list()[0]
            _new_codons = df.loc[(df['position'] == _position_in_protein) & (df['mutant_aa'] == _new_amino_acid) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)]['mutant_codon'].to_list()
            _new_codon = _new_codons[0]
            _observed_codon_counts = df.loc[(df['position'] == _position_in_protein) & (df['mutant_aa'] == _new_amino_acid) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)]['observed_codon_count'].to_list() # fetch coverage counts for all synonymous codons
            _observed_codon_count_sum = sum(_observed_codon_counts)
            _total_codons_per_site = df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _new_codon)]['total_codons_per_site'].to_list()
        ...
        def on_add(sel):
            ypos, xpos = int(sel.target[1]), int(sel.target[0])
            print(f"Info: xpos={xpos}, ypos={ypos}, calculated_aa_offset={calculated_aa_offset}")
            _new_codon = new_codon_table.index[ypos]
            #_old_codon = old_codon_table.index[ypos]
            _position_in_protein = new_codon_table.columns[xpos+1-myoptions.offset-calculated_aa_offset]
            _frequency = new_codon_table.at[_new_codon, _position_in_protein]
            print(f"Info: _position_in_protein={_position_in_protein}, _frequency={_frequency}")
            _old_codon = df.loc[df['position'] == _position_in_protein]['original_codon'].to_list()[0]
            _old_amino_acid = df.loc[df['position'] == _position_in_protein]['original_aa'].to_list()[0]
            #_new_codon = df.loc[(df['position'] == _position_in_protein)][xpos-myoptions.offset].to_list()['mutant_codon']
            _observed_codon_count = df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _new_codon) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)]['observed_codon_count'].to_list()[0]
            _total_codons_per_site = df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _new_codon) & (df[myoptions.column_with_frequencies] >= myoptions.threshold)]['total_codons_per_site'].to_list()[0]
            # print relevant lines from df matching a particular codon column
            print(f"Info: {len(df.loc[df['position'] == _position_in_protein]['position'])} aa residues observed in position {_position_in_protein}:{os.linesep} {str(df.loc[df['position'] == _position_in_protein][0:])}{os.linesep}")
            #print(f"Info: Looking for _new_codon={_new_codon}")
            try:
                _new_amino_acid = df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _new_codon)]['mutant_aa'].to_list()[0]
            except:
                _new_amino_acid = "Failed to find, the %s is wrong and too far from original codon" % _new_codon
            _score = matrix[_old_amino_acid][_new_amino_acid]
            try:
                _some_frequency = Decimal(df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _new_codon)][myoptions.column_with_frequencies].to_list()[0]) # convert strings to Decimal values
            except:
                _some_frequency = 0.00000000009 # enter junk value but prevent matplotlib from crashing
            if _some_frequency != _frequency and (_some_frequency != 0.00000000009 and _frequency != 0) and not np.abs(_frequency) < myoptions.threshold:
                raise ValueError("Frequency new_codon_table.at[_new_codon, _position_in_protein]=%s not same as df.loc[(df['position'] == _position_in_protein) & (df['mutant_codon'] == _new_codon)][myoptions.column_with_frequencies].to_list()[0]=%s" % (_frequency, _frequency))

            if myoptions.debug:
                print(f"Debug: Position: {_position_in_protein }\nOriginal Codon: {_old_codon} ({_old_amino_acid})\nNew Codon: {_new_codon} ({_new_amino_acid})\n{matrix_name} score: {_score}\nFrequency: {_frequency:.6f}")
    ...
    # Draw legend with circles scaled to the main drawing. For that we need to render fake data points with the examples widths outside the visible axis.
    _handles, _labels = [], []
    if myoptions.aminoacids:
        _junk = 'NNN'
        _codon_on_input = False
    ...
    for _freq in [0.001, 0.01, 0.1, 0.3]:
        if myoptions.column_with_frequencies in ['neutralized_parent_difference', 'escape_parent_difference']:
            _size, _color = adjust_size_and_color_neutralized_escape(Decimal(_freq))
            # _size = _size * 5000
        elif myoptions.column_with_frequencies in ['weighted_diff_escape_neutralized']:
            _size, _color = adjust_size_and_color_weighted(Decimal(_freq), _codon_on_input) # size is already multiplied by either 2000 or 3000 or 5000
        else:
            _score, _size, _color = adjust_size_and_color(Decimal(_freq), _codon_on_input, _junk, _junk, _junk, _junk, matrix, norm, colors)
            # _size = _size * 5000
    ...
    #_box = ax4.get_position()
    #ax4.set_position([_box.x0, _box.y0, _box.width * 1.00, _box.height * 1.00])
    ax4.set_axis_off()
    #plt.xticks(rotation=90)
...
def main():
    _matrix, _matrix_name, _min_theoretical_score, _max_theoretical_score, _outfile_prefix = load_matrix()

    _df, _title_data, _aln_rows = load_and_clean_dataframe(_outfile_prefix)
    ...
    (
        ...
        _old_aa_table, _new_aa_table, _old_codon_table, _new_codon_table,
        _calculated_aa_offset,
    ) = build_frequency_tables(_df)

    _figure, _ax1, _ax2, _ax3, _ax4, _xmin, _xmax = setup_matplotlib_figure(
        _title_data, _aln_rows, _matrix_name, _amino_acids, _codons_whitelist,
        _final_sorted_whitelist, _unique_aa_positions, _unique_codon_positions,
    ...
    (
        ...
        _df, _table, _outfile_prefix, _matrix, _amino_acids, _codons_whitelist2,
    )

    _xlabel = _ax1.get_xlabel()

    render_bokeh(
        _outfile_prefix, _xmin, _xmax, _amino_acids, _final_sorted_whitelist,
    ...
    render_matplotlib(
        ...
        _new_aa_table, _new_codon_table, _df, _codons_whitelist2, _final_sorted_whitelist,
        _calculated_aa_offset,
    )
