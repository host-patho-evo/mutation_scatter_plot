...
| `--extract-discarded-fasta` | For each child file with a `.discarded_sha256_hashes.txt`, extract the corresponding original FASTA records from the root ancestor FASTA. The lookup uses the first GISAID-level ancestor's `sha256_to_ids.tsv` (mapping sha256 → original accession IDs). Output: `{child_stem}.discarded_original_entries.fasta`. This step uses a highly-optimized native Python byte-streaming extractor, completely replacing the legacy `filterbyname.sh` dependency to guarantee encoding stability. |
...
**The Python streaming replacement:**

As of recent refactors, the pipeline **completely dropped `filterbyname.sh`** and all BBTools extraction wrappers in favor of a native Python byte-streaming crawler for `--extract-discarded-fasta`. Because the native pipeline uses pure I/O byte matching across `surrogateescape`-decoded lines, it structurally ignores Java's internal literal Unicode expansion logic. This natively bypasses all three failure modes above and elegantly handles control characters as benign bytes alongside geographical spaces.
