...
def _process_one_site(
    ...
    for (_rough_sample_codon, _action), _record_count in _local_groups.items():
        ...
        if _rough_sample_codon == '---':
            _sample_codon_depadded = '---'
        elif _sample_codon_contained_pad:
...
def parse_alignment(myoptions: typing.Any, alignment_file: str, padded_reference_dna_seq: str,
    ...
    if _slim_positions:
        ...
        for _res in _all_results:
            # Write results
            _resolved_ref_aa = 'INS' if _res['ref_codon'] == '---' else _res['ref_aa']
            write_tsv_line(outfilename_unchanged_codons, _res['unchanged'], _res['nat_padded'], _res['nat_depadded'],
                           _resolved_ref_aa, _res['total_sum'], _res['ref_codon'], debug=myoptions.debug, translation_table=translation_table)  # noqa: E501
            if _res['changed']:
