...
# ── Shared scoring / coloring / matrix logic ────────────────────────────
# These functions are the single source in core.py; re-exported here so that
# all existing ``from mutation_scatter_plot.mutation_scatter_plot import …``
# paths keep working.  The core module is also imported directly by
# mutation_timeline_plot to avoid pulling in the heavy rendering stack.
from .core import (  # noqa: E402  — must come after module-level alt_translate
    get_colormap,
    resolve_codon_or_aa,
    get_score,
    adjust_size_and_color,
...
from . import core as _core  # direct module reference for mutable globals
...
def build_frequency_tables(myoptions, df, padded_position2position):
    ...
    # INS summary row for aminoacids mode: aggregate all original_aa=='INS'
    # entries by position into the 'INS' row.  The normal groupby(['mutant_aa',
    # 'padded_position']) scatters INS events into their actual mutant_aa rows
    # (V, M, P, …), leaving the 'INS' row empty.
    if myoptions.aminoacids and 'INS' in _amino_acids and 'INS' in _new_aa_table.index:
        _ins_df = _filtered_df[_filtered_df['original_aa'] == 'INS']
        if not _ins_df.empty:
            _ins_sums = _ins_df.groupby('padded_position')[_freq_col].sum()
            for _pos, _val in _ins_sums.items():
                if _pos in _new_aa_table.columns:
                    _new_aa_table.at['INS', _pos] = Decimal(_val)
...
def setup_matplotlib_figure(
    ...
    if myoptions.disable_padded_x_axis:
        unique_aa_padded_positions = [padded_position2position.get(p, p) for p in unique_aa_padded_positions]
        unique_padded_codon_positions = [padded_position2position.get(p, p) for p in unique_padded_codon_positions]
    ...
    if myoptions.aminoacids:
        if myoptions.disable_padded_x_axis:
            if myoptions.shortlegend:
                ...
                _xlabel = f'Amino acid position{os.linesep}based on {aln_rows.strip(os.linesep)} ALN rows, matrix {matrix_name}, colormap {myoptions.colormap}, mutation_scatter_plot {VERSION}'
        else:
            if myoptions.shortlegend:
        ...
        if myoptions.disable_padded_x_axis:
            if myoptions.shortlegend:
                ...
                _xlabel = f'Codon position{os.linesep}based on {aln_rows.strip(os.linesep)} ALN rows, matrix {matrix_name}, colormap {myoptions.colormap}, mutation_scatter_plot {VERSION}'
        else:
            if myoptions.shortlegend:
    ...
    _labelsize = 10 if myoptions.disable_padded_x_axis else 8

    if myoptions.aminoacids:
        _ax1.xaxis.set_tick_params(labelsize=_labelsize)
        _ax1.tick_params(axis='x', which='both', labelsize=_labelsize)
...
def collect_scatter_data(
    ...
    for _, row in df[['original_codon', mut_col]].drop_duplicates().iterrows():
        ...
        if _new in ('---', 'DEL', 'INS'):
            _emp_scores.add(_core._min_theoretical_score)
        else:
            try:
    ...
    for i, _some_codon_or_aa in enumerate(table.index):  # so _some_codon_or_aa contains the index specified when the table was constructed
        for j, _padded_position in enumerate(table.columns):  # so _aa_position contains the real aa_position
            ...
            # Special case: INS summary row in aminoacids mode.
            # Same logic as the +++ codon handler below, but for the 'INS' amino
            # acid row.  The aa table's 'INS' row was explicitly populated by
            # build_frequency_tables from original_aa=='INS' events.
            if _some_codon_or_aa == 'INS' and myoptions.aminoacids:
                _frequency = table.loc['INS', _padded_position]
                ...
                if abs(Decimal(_frequency)) < myoptions.threshold:
                    _dots.append((_x_coord, i, 0.00000000009, 'dot', '#000000', 0.5, _core._min_theoretical_score))
                    continue
                _ins_at_pos = df[
                ...
                _color = '#ff0000'
                _score = _core._min_theoretical_score
            ...
            if not abs(Decimal(_frequency)) < myoptions.threshold:
                ...
                if myoptions.aminoacids:
                    ...
                    if _score < 0:
                        _circles_bokeh.append((_x_coord, _some_codon_or_aa, _bokeh_size, 'circle',
                                              _color, 0.5, _score, _aa_position, _padded_position))
                    else:
                        _circles_bokeh.append((_x_coord, _some_codon_or_aa, _bokeh_size, 'hex',
                                              _color, 0.5, _score, _aa_position, _padded_position))
                else:
                    ...
                    if _score < 0:
                        ...
                        _circles_bokeh.append((_x_coord, _y_label_bokeh, _bokeh_size, 'hex',
                                              _color, 0.5, _score, _aa_position, _padded_position))
                ...
                if _score < 0:
                    _circles_matplotlib.append((_x_coord, i, _mpl_s, 'circle_x', _color, 0.5,
                                               _score, _aa_position, _padded_position, _hover_text))
                    _markers.append((_x_coord, i, 1, 'dot', '#000000', 0.5))    # black cross marker for negative-score circles
                else:
                    _circles_matplotlib.append((_x_coord, i, _mpl_s, 'circle', _color, 0.5,
                                               _score, _aa_position, _padded_position, _hover_text))
                    _markers.append((_x_coord, i, 1, 'circle', '#000000', 0.5))  # black rim marker for non-negative circles
                _used_colors.add(_color)
                ...
                _dots.append((_x_coord, i, _size, 'dot', _color, 0.5, _score))
