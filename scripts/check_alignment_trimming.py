...
def _extract_sha256(record_name):
    """Extract 64-char hex sha256 from NNNNx.sha256 ID, or None."""
    dot = record_name.find(".")
    if dot > 0 and len(record_name) >= dot + 65:
        candidate = record_name[dot + 1: dot + 65]
...
def main():
    """Read the aligned FASTA and report sha256 mismatches."""
    n_total = n_with_sha = n_mismatch = 0
    current_name = None
    seq_parts = []

    def _check(check_name, check_parts):
        nonlocal n_with_sha, n_mismatch
        sha_in_id = _extract_sha256(check_name)
        if sha_in_id is None:
            return  # legacy ID without sha256 — skip
        n_with_sha += 1
        seq = "".join(check_parts).replace("-", "").upper()
        sha_computed = hashlib.sha256(seq.encode()).hexdigest()
        if sha_computed != sha_in_id:
            n_mismatch += 1
            if args.debug:
                orig_len = len("".join(check_parts).replace("-", ""))
                print(f"MISMATCH id={check_name}  id_sha={sha_in_id}"
                      f"  seq_sha={sha_computed}  depadded_len={orig_len}")
    ...
    if n_with_sha:
        print(f"  Match rate        : {(n_with_sha - n_mismatch) / n_with_sha * 100:.4f}%")
    else:
        print("  N/A")
