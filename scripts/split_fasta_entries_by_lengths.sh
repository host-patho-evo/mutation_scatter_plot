...
find . -name \*fastp.amplicons.clean.counts.fasta | sort | while read f; do p=`echo $f | sed -e 's/.fasta//'`; split_fasta_entries_by_lengths --infile="$f" --outfile-prefix="$p" --full-length=3822; done
