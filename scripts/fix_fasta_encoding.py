...
This script uses Python's ``surrogateescape`` error handler instead:
  ...
  Step 4 — C0 control character stripping (and TAB conversion):
      ...
      **BBTools / Java Encoding Corruption (filterbyname.sh)**
      ...
      platform character set. If the cluster environment lacks a strict Unicode
      definition (e.g. an unset ``LC_ALL`` environment variable or missing
      ``JAVA_TOOL_OPTIONS="-Dfile.encoding=UTF-8"`` parameter), the JVM dynamically
      falls back to basic ANSI / ISO-8859-1 code pages.

      If a GISAID FASTA file contains pure UTF-8 encoded multi-byte characters
      (e.g., the two-byte sequence ``\\xC3\\xAD`` for ``í`` in ``Microbiología``),
      passing it through ``filterbyname.sh`` forces this ANSI-limited Java pipeline
      to decode the bytes assuming Latin-1. Because the bytes do not logically span
      as UTF-8 in that locale, the JVM catastrophically shreds the byte sequences,
      arbitrarily casting mismatched bounds into the Unicode Replacement Character
      (``\uFFFD`` / byte 0xFD).

      During sequence extraction, a perfectly valid ``Microbiología`` subsequently
      becomes silently and brutally corrupted to ``Microbiologýýa`` on output.

      Because BBTools intrinsically modifies the textual bytes during write-out
      based strictly on the host environment's ``LC_ALL`` variables, the downstream
      python tracking systems—which reconstruct traceabilities via byte-for-byte
      matching between FASTA files—completely fail.
